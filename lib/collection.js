...
Collection.prototype.insert = function(object) {
    ...
    var that = this;
    this.validate(object, true).then(function(valid) {
        if (!valid) {
            deferred.reject({
                status: 400,
        ...
        that.collection().insert(object || {}, function(err, docs) {
            if (err) {
                deferred.reject(err);
            } else {
                that.filter(docs[0]).then(function() {
...
Collection.prototype.findAndModify = function(objectID, set) {
    var deferred = Q.defer();
    var that = this;
    this.validate(set, false).then(function(valid) {
        if (!valid) {
            deferred.reject({
                status: 400,
        ...
        that.collection().findAndModify({
            ...
            if (err) {
                ...
                that.filter(object).then(function() {
...
/**
 * Validates the input when a document is inserted or updated. The validator can modify the received document.
 ...
 * If false, is being updated. Returns true/false if the document is valid/invalid (or a promise)
 */
