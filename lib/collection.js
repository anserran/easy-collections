...
Collection.prototype.insert = function(object) {
    ...
    this.collection().insert(object || {}, function(err, docs) {
        if (err) {
            ...
            that.filter(docs[0]).then(function(doc) {
...
Collection.prototype.findAndModify = function(objectID, set) {
    ...
    this.collection().findAndModify({
        ...
        if (err) {
            ...
            that.filter(object).then(function(object) {
 ...
 * @param {Function} filter receives a document, and returns a promise that resolves with the document filtered.
 */
